...
with tabs[0]:
    st.header("🔴 Live Quality Monitoring Dashboard")
    
    if not ANALYTICS_AVAILABLE:
        st.error("Real-time monitoring not available. Please ensure all dependencies are installed.")
        st.stop()
    ...
    with col1:
        monitoring_active = st.checkbox("🔴 Enable Live Monitoring", value=False, key="live_monitoring")
    
    with col2:
        refresh_interval = st.selectbox("Refresh Interval", [30, 60, 120, 300], index=1, key="refresh_interval")
    ...
    try:
        ...
        monitoring_manager = create_monitoring_manager(client)
        
        # Get live monitoring data
        live_data = monitoring_manager.get_live_dashboard_data()
        
        if 'error' in live_data:
            st.error(f"Monitoring Error: {live_data['error']}")
        else:
            # Display live status
            status_col1, status_col2, status_col3 = st.columns(3)
            
            with status_col1:
                status = live_data.get('monitoring_status', 'inactive')
                status_color = "🟢" if status == 'active' else "🔴" if status == 'error' else "🟡"
                st.markdown(f'''
                <div class="kpi-card">
                    <div class="kpi-label">{status_color} Monitoring Status</div>
                    <div class="kpi-value">{status.title()}</div>
            ...
            with status_col2:
                ...
                st.markdown(f'''
                <div class="kpi-card">
                    <div class="kpi-label">{alert_color} Active Alerts</div>
            ...
            with status_col3:
                last_update = live_data.get('timestamp', datetime.now())
                time_str = last_update.strftime("%H:%M:%S") if isinstance(last_update, datetime) else str(last_update)[:8]
                st.markdown(f'''
                <div class="kpi-card">
                    <div class="kpi-label">🕐 Last Update</div>
                    <div class="kpi-value">{time_str}</div>
                    <div class="kpi-label">Real-time Data</div>
                </div>
                ''', unsafe_allow_html=True)
            
            # Live Metrics Section
            st.subheader("📊 Live Quality Metrics")
            
            live_metrics = live_data.get('live_metrics', {})
            current_values = live_metrics.get('current_values', {})
            
            if current_values:
                metric_cols = st.columns(len(current_values))
                
                for idx, (metric_name, metric_data) in enumerate(current_values.items()):
                    with metric_cols[idx]:
                        ...
                        if 'quality' in metric_name.lower():
                            ...
                            color = "🟢" if value >= 50 else "🟡" if value >= 20 else "🔴"
                        else:
                            color = "🔵"
                        
                        st.markdown(f'''
                        <div class="kpi-card">
                            <div class="kpi-label">{color} {display_name}</div>
            ...
            # Live Trends Charts
            st.subheader("📈 Live Trend Analysis")
                ...
                for metric_name, trend_data in trends.items():
                    if trend_data:
                        ...
                        fig = px.line(
                            ...
                            x='timestamp', 
                            y='value',
                            color='category' if 'category' in df_trend.columns else None,
                            title=f"Live {metric_name.replace('_', ' ').title()} Trend",
            ...
            if live_data.get('active_alerts'):
                st.subheader("🚨 Active Alerts")
                
                for alert in live_data['active_alerts']:
                    severity = alert.get('severity', 'info')
                    alert_type = st.error if severity == 'critical' else st.warning if severity == 'warning' else st.info
                    
                    alert_type(f"**{alert.get('message', 'Alert triggered')}** - {alert.get('timestamp', 'Unknown time')}")
            ...
            if system_health:
                ...
                with health_col1:
                    ...
                    status_color = "🟢" if health_status == 'healthy' else "🟡" if health_status == 'warning' else "🔴"
                    st.markdown(f"{status_color} **{health_status.title()}**")
                ...
                with health_col2:
                    ...
                    for comp_name, comp_data in components.items():
                        ...
                        comp_color = "🟢" if comp_status == 'healthy' else "🟡" if comp_status == 'warning' else "🔴"
                        st.markdown(f"{comp_color} **{comp_name.title()}**: {comp_status}")
                        
                        if 'message' in comp_data:
                            st.caption(comp_data['message'])
            
            # Auto-refresh logic
            if auto_refresh and monitoring_active:
        ...
        st.markdown("- Ensure BigQuery connection is active")
        st.markdown("- Verify data pipeline is running")
        st.markdown("- Check system permissions")
